...
description: "Transform a user-supplied photo into a Gathered Scenes Zine poster while routing the enlarged source-derived illustration field through a user-specified visual style. Preserve the original skill's truthful photographic anchor, spacious abstraction, one structural high-chroma hue, visible hand-torn fibrous transition, negative space, and restrained micro-text. Route requests for 动漫风/anime, 极简线稿/minimal line art, 赛博朋克/cyberpunk, or 中国古风/Chinese classical without changing the underlying collage system. Use whenever the user asks to stylize a supplied photo in one of these styles while keeping the tactile gathered-scenes poster language; if no route is specified, use the original Gathered Scenes visual language."
---
...
Resolve the requested style before compiling the prompt. This is a thin overlay on the original Gathered Scenes system: it changes only the **illustration field's primary grammar, mark-making, color relationship, and reproduction texture**. It does not change the truthful photographic anchor, source-derived composition, complexity compression, one-added-hue rule, negative-space targets, torn-paper handoff, micro-text system, output format, or any other instruction in this skill.

If the user does not specify a route, use the original Gathered Scenes visual language unchanged. If the user supplies exact style, substyle, color, or text instructions, follow them within the limits above. If several styles are named, use the first clearly requested style as primary and treat the rest as subordinate modifiers only when they do not create a mixed-style result.

Recognize these routes and common Chinese or English variants:

- '动漫 / 二次元 / 动画 / anime / animated film' → **Anime illustration**
- '极简线稿 / 线稿 / 线条画 / line art / minimal sketch' → **Minimal line art**
- '赛博朋克 / cyberpunk / neon / neon city' → **Cyberpunk print**
- '中国古风 / 古风 / 国风 / 水墨 / 工笔 / 青绿山水 / ink wash / guofeng' → **Chinese classical illustration**

Do not imitate a named living artist or a specific studio. Translate such references into visible, general attributes such as hand-painted animation, translucent backlight, restrained ink contour, mineral color, or nocturnal neon print.

### Anime Illustration
Keep the photographic anchor truthful. Render only the enlarged illustration field with an original animated-film language:

- choose **silhouette-led** or **cut-paper-led** grammar, supported by a few clean broken contours;
- merge clouds, foliage, reflections, clothing folds, and architecture into broad hand-painted or cel-like color masses;
- use soft diffuse light, restrained edge variation, and simplified environmental storytelling rather than photographic shading;
- let the one added hue behave as a source-derived flat color passage or underprint, not a detached bright effect;
- preserve the cream paper, torn fibers, low-density illustration field, and quiet micro-text.
For this route only, the global avoid on anime treatment does not apply to the illustration field. Still avoid cute-character conversion, studio imitation, plastic 3D rendering, photographic outline filters, exaggerated facial redesign, and excessive bloom.

### Minimal Line Art
Keep the photographic anchor truthful. Render the enlarged illustration field with **contour-led** grammar:

- preserve only one or two defining outlines, the dominant directional gesture, and one source-specific structural rhythm;
- use broken black, charcoal, dark brown, or single-hue ink lines with slight pressure variation;
- omit realistic shading, surface texture, closed descriptive outlines, and most internal detail;
- allow one source-derived flat counterform or underprint in the existing single added hue;
- increase internal blank paper rather than filling the field with more lines.
Avoid automatic grayscale edge tracing, dense hatching, scratchy doodle noise, full-object contouring, and line density that weakens the original quiet-space targets.

### Cyberpunk Print
Keep the photographic anchor recognizable and naturally textured; do not replace it with a fully synthetic city. Render the enlarged illustration field as a **field-led** nocturnal print:

- preserve the source skyline, road, window, bridge, figure, or perspective axis that defines the scene;
- use deep blue-black or charcoal neutrals plus exactly one added high-chroma hue, choosing cyan, magenta, ultraviolet, or electric red according to the source;
- attach neon light to a real window, sign, road, reflection, vehicle, garment, or architectural rhythm;
- express glow through halftone, risograph misregistration, broken ink, and restrained underprint rather than glossy digital bloom;
- keep technology details sparse and source-justified.
For this route only, the global avoids on neon and cinematic lighting do not prohibit a restrained source-anchored neon atmosphere in the illustration field. Still avoid multiple competing neon hues, illegible signs, unrelated futuristic props, full-screen glow, glossy 3D rendering, and game-poster spectacle.

### Chinese Classical Illustration
Keep the photographic anchor truthful. Render the enlarged illustration field through one Chinese classical grammar selected from the user's request or the source atmosphere:

- **ink wash:** field-led ink masses, dry-brush edges, graded wash, and large paper voids;
- **gongbi:** a few fine controlled contours with restrained flat color, never comprehensive detail;
- **blue-green landscape:** simplified mineral-blue and mineral-green planes, while the single added hue remains the only new high-chroma structural color;
- **literati light color:** sparse ink gesture with quiet ochre or blue-green support.
Preserve the source's dominant lean, ridge, water path, roofline, figure direction, or spatial opening rather than inventing generic mountains, pavilions, boats, birds, or calligraphy. Keep the standard micro-text system unless the user explicitly supplies Chinese wording or requests a Chinese-only or bilingual mode.
Avoid a generic guofeng filter, decorative motifs unrelated to the source, full-scene antique repainting, dense brushwork, ornate seals, saturated red-green competition, and the loss of the photographic anchor.

### Route Integration
Apply the chosen route when selecting the primary illustration grammar, compiling the abstraction map, choosing the exact added hue and material, specifying reproduction texture, and writing the final prompt. Route-specific exceptions override only the explicitly named conflicting phrase in the Hard Avoids; every other avoid and every other section remains in force.

Before generation, verify that the selected route is visible in the illustration field without changing the poster into a different product category. At thumbnail size it must still read first as a Gathered Scenes photo–illustration collage, and only then as anime, line art, cyberpunk, or Chinese classical.
